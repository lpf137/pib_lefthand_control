...
CONFIGS = {
    "sports ball": {
        ...
        "angle_min": 60.0,
        "angle_max": 77.0,
        "prep_angle_offset": -10.0,
        "arm_prep_func": lambda ang: [(0x10, 20), (0x6, ang), (0x3, 85), (0x9, 88)],
        "hand_seq": [
            [(10, 50)],
            [(11, 360), (9, 579), (8, 850), (6, 750)],
        ],
...
class SpatialVisualizer(dai.node.HostNode):
    def __init__(self):
        ...
        self.target_labels = ["sports ball"]
    ...
    def map_x_to_angle(self, x_mm: float, label: str) -> float:
        cfg = CONFIGS.get(label, CONFIGS["sports ball"])
    ...
    def process(self, depthPreview, detections, rgbPreview):
        ...
        candidates = [best_by_label[label] for label in self.target_labels if label in best_by_label]
        if candidates:
    ...
    def _bg_grasp_sequence(self, current_angle_deg: float, label: str):
        ...
        try:
            ...
            # 2. 灵巧手抓握（支持分阶段执行）
            hand_sequences = cfg.get("hand_seq", [])
            if hand_sequences:
                if hand_sequences and isinstance(hand_sequences[0], tuple):
                    hand_sequences = [hand_sequences]
                for seq in hand_sequences:
                    hand_grasp_execute(seq, time_ms=3000, delay_s=1.0)
